...
private:
  ...
  bool amrMcuServiceCallback(std_srvs::SetBool::Request &req,
                             std_srvs::SetBool::Response &res)
  {
    ROS_INFO("[amr_go_move] /amr_mcu_service request: %s",
             req.data ? "true" : "false");

    // If request is false, do nothing.
    if (!req.data)
    {
      res.success = true;
      res.message = "Request is false; no action.";
      return true;
    }

    // 1) Run ArUco-based precise approach
    callGoAruco(true);
    
    callAmrMcu(0, "PLACE", 1);
    
    // 2) Publish mission state "UNLOADING"
    std_msgs::String state_msg;
    state_msg.data = "UNLOADING";
    mission_pub_.publish(state_msg);
    ROS_INFO("[amr_go_move] Published amr_mission_state = 'UNLOADING'");
  ...
  void handleGostandbyCommand()
  {
    ROS_INFO("[amr_go_move] Handling move_command: go_standby");

    // Navigate to standby position
    bool ok = sendGostandbyGoal();
    if (!ok)
    {
      ROS_WARN("[amr_go_move] go_standby navigation failed.");
    }

    // Disable DB logger (even if navigation failed)
    callSetStatus(false, "LOGGER_OFF", 1);

    // Optional post-standby ArUco alignment + MCU reset + relative rotation
    if (callGoAruco(true))
    {
  ...
  void opcuaMoveCallback(const std_msgs::String::ConstPtr &msg)
  {
    ...
    */
    ...
    else
    {
      ...
      else if (data.find("go_standby") != std::string::npos)
      {
        effective_cmd = "go_standby";
      }
      else if (data.find("unloading") != std::string::npos)
      {
        effective_cmd = "unloading";
      }
    }
    ...
    else if (effective_cmd == "go_standby")
    {
      handleGostandbyCommand();
    }
    else if (effective_cmd == "unloading")
    {
      if (!callAmrMcu(2, "PLACE", 1))
      {
        ROS_WARN("[amr_go_move] callAmrMcu(2, 'PLACE', 1) failed.");
        return;
      }
  ...
  // Logical destinations (pick-up, standby)
  double pick_up_zone_x_;
  double pick_up_zone_y_;
  double pick_up_zone_yaw_deg_;

  double go_standby_x_;
  double go_standby_y_;
  double go_standby_yaw_deg_;
