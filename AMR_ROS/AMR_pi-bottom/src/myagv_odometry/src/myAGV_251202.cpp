...
bool MyAGV::init()
{
    ...
    external_imu_received = false;  // 추가
    ...
    sub_external_imu = n.subscribe("/external_imu", 10, &MyAGV::imuCallback, this);
...
{
    external_imu_data = *msg;
    external_imu_received = true;
    
    // 디버깅용 로그 (필요시)
    // ROS_INFO_THROTTLE(1.0, "External IMU received: yaw angular vel = %.3f", 
