...
def _shutdown_cb(msg: Bool):
    ...
    if msg.data:
        rospy.loginfo("[camera_pub_node] shutdown signal received.")
        _shutdown_flag = True
...
def main():
    ...
    rospy.loginfo(f"camera_pub_node started (cam={CAM_INDEX}, {WIDTH}x{HEIGHT}@{FPS}fps)")
    ...
    rospy.loginfo("camera_pub_node stopped.")
