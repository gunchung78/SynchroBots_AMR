...
class ImuOdomFusionNode(object):
    ...
    def odom_callback(self, odom_msg):
        ...
        # ----- header -----
        # stamp: 그대로 쓰거나, 현재 시간으로 갱신
        fused.header.stamp = odom_msg.header.stamp
        # 입력 odom frame을 그대로 쓰되, 필요하면 "odom"으로 고정해서 사용해도 됨
        fused.header.frame_id = "odom"

        # child_frame_id: 기존 값 사용 (없으면 base_footprint로 디폴트)
        ...
        # orientation만 IMU 값으로 교체
        fused.pose.pose.orientation = self.latest_imu_orientation

        # (선택) covariance 그대로 복사
        fused.pose.covariance = odom_msg.pose.covariance
        fused.twist.covariance = odom_msg.twist.covariance
